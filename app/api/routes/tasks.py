...
from app.api.dependencies import get_db, get_current_user
from app.models.user import User
from app.utils.authz import is_admin_or_self  # ✅ Add this
...
def get_task(task_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    if not is_admin_or_self(current_user, task.owner_id):
...
def update_task(task_id: int, task_update: TaskUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    task = db.query(Task).filter(Task.id == task_id).first()
    if not task or not is_admin_or_self(current_user, task.owner_id):
